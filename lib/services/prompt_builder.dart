...
또한 대화 내용을 바탕으로, 사용자가 **지금 바로 실행할 수 있는 구체적인 다음 행동**을 떠먹여주듯 안내해주세요.
"~하면 좋습니다" 같은 막연한 조언이 아니라, 검색 키워드·설치 명령어·읽어볼 링크·따라할 순서까지 포함해야 합니다.
...
4. **톡비서의 의견**: 대화 내용을 바탕으로 사용자가 **지금 당장 해볼 수 있는 구체적 행동**을 제안
   - "~해보세요" 같은 막연한 조언 절대 금지. 반드시 **어디서, 뭘 검색하고, 뭘 읽고, 뭘 설치하고, 어떤 순서로 하는지** 단계별로 떠먹여주기
   - 대화에서 언급된 도구/기술/링크와 직접 연결해서 작성 (대화와 무관한 일반론 금지)
   - 잘못되었거나 함정이 있는 정보는 "⚠️ 주의:" 로 먼저 짚고, 올바른 방향 제시
   - 초보자 기준으로 작성: "이미 알겠지만~" 식 생략 없이, 검색 키워드/명령어/설정값까지 구체적으로
   - 예시 형식:
     "대화에서 나온 Ollama로 로컬 LLM 돌리는 것에 관심 있다면:
     ① ollama.com 에서 설치 파일 다운로드 → 설치 후 터미널에서 `ollama run llama3.2` 입력
     ② 본인 RAM이 16GB면 8B 모델까지만 쾌적. `ollama list`로 설치된 모델 확인
     ③ 대화에서 추천된 'open-webui'를 같이 쓰면 ChatGPT 같은 웹 UI로 사용 가능 → GitHub에서 'open-webui' 검색해서 Docker로 설치"
